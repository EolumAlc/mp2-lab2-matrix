...
TVector<ValType>::TVector(int s, int si) {
    if (s < 0 || s > MAX_VECTOR_SIZE) {
        throw "Size TVector(int s, int si)";
    }
    if (si < 0) {
        throw "StartIndex TVector(int s, int si)";
    }
    Size = s;
    StartIndex = si;
...
ValType &TVector<ValType>::operator[](int pos) {
    /*  // s = 7, si = 5, pos = 8; if (pos - si < 0 || pos >= s + si)
                            //0 1 2 3 4 5  6
                            //5 6 7 8 9 10 11
    */
    if (pos - StartIndex < 0 || pos >= Size + StartIndex) {
        throw "operator[](int pos)";
    }
...
TMatrix<ValType>::TMatrix(int s): TVector<TVector<ValType> >(s) {
    if (s < 0 || s > MAX_MATRIX_SIZE) {
        throw "TMatrix(int s): TVector<TVector<ValType> >(s)";
    }
    for (int i = 0; i < s; ++i) {
